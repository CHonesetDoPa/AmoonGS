...
ipcMain.handle('run-command', async (_event, command) => {
    return new Promise((resolve, reject) => {
        ...
        if (command === 'start gnirehtet.exe run') {
            process = spawn(path.join(__dirname, 'gnirehtet.exe'), ['run'], { stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
        } else {
            process = spawn(command, { shell: true, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
        }

        process.stdout.on('data', (data) => {
            const message = `✅ 执行成功: ${data.toString()}`;
            if (command.includes('gnirehtet')) {
                mainWindow.webContents.send('command-result', `gnirehtet: ${command}: ${message}`);
            } else {
